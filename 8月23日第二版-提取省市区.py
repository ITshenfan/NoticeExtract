...
def select_datasource():
    read_path = "DataSourceFile/sp_govs.xls"
...
sql = """CREATE TABLE Data1 (ID INT PRIMARY KEY AUTO_INCREMENT,
                                  公告级别id INT(11),
                                  公告标签 TEXT,
                                  省份 VARCHAR(255),
                                  地市 VARCHAR(255),
...
def savedata():
    ...
    for key in firstresult:
        if (get_proxy(key.url) != None):
            ...
            sqlw = """INSERT INTO Data1 (公告级别id,公告标签, 省份,地市,区县,项目类型,公告类型,编制情况,公告名称,公告链接,公告原网链接,公告父链接,招录人数,招录岗位数,公告发布时间,公告正文,附件链接) VALUES (%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%d,%s,%s,%s)"""
