...
# 判断有效的title
def useful_title(setArray,status):
    for key in setArray:
        if ('无障碍' in key.title):
            return None
            ...
            if (title is not None):
                ...
                if (get_proxy(url) != None):
                    ...
                    if (isNotValidLink):
                        ...
                        # 在这里返回有效的结点信息
                        return url, title,status,key.url
            else:
                return None
...
def getresultFromSecondresult(setArray):
    if(useful_title(setArray, 2) != None):
        url,title,status,preurl = useful_title(setArray,2)
        if(url is not  None):
            addnode(url,title,status,preurl)

# 当前的所有链接中，可能还有有效的文本，也就是4级别，这个仅判断firstresult即可
def getresultFromMoreresult(setArray):
    if(useful_title(setArray, 3) != None):
        url,title,status,preurl = useful_title(setArray,3)
        if(url is not  None):
            addnode(url,title,status,preurl)

def getresultFromFirstresult(setArray):
    if(useful_title(setArray, 4) != None):
        url,title,status,preurl = useful_title(setArray,4)
        if(url is not  None):
            addnode(url, title, 4, preurl)
